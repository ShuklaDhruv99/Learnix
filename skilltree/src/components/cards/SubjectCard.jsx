...
export default function SubjectCard({ subject, onEnroll }) {
  const Icon = getIcon(subject.icon || 'BookOpen')
  const a = accentMap[subject.accent] || accentMap.emerald

  const content = (
    <Card hover glow={subject.accent} className="p-5 group h-full relative">
      <div
      ...
      <p className="text-xs text-white/40 mb-4 relative">
        {subject.topics_completed} / {subject.topics_total} topics
      </p>
      <div className="flex items-center gap-2 flex-wrap relative">
        <Badge accent={subject.accent}>{subject.difficulty}</Badge>
        <span className="inline-flex items-center gap-1 text-xs text-white/40">
          <Clock className="w-3.5 h-3.5" /> {subject.estimated_hours}h
        </span>
        <span className="inline-flex items-center gap-1 text-xs text-white/40 ml-auto font-mono">
          <Zap className="w-3.5 h-3.5 text-gold-bright" /> {subject.xp_earned.toLocaleString()}
        </span>
      </div>
      {!subject.is_enrolled && (
  ...
  if (!subject.is_enrolled) {
    return content // not clickable into the skill tree until enrolled
  }

  return <Link to={`/app/skill-tree?subject=${subject.id}`}>{content}</Link>
}